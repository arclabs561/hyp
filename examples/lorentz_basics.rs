...
fn main() {
    ...
    println!("Minkowski norm of origin: {:.4} (should be -1/c = -1.0)", inner);
    println!("Origin on manifold? {}\n", model.is_on_manifold(&origin.view(), 1e-6));
    ...
    let err: f64 = p.iter().zip(p_back.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f64>().sqrt();
    println!("  point:     [{:.6}, {:.6}, {:.6}]", p[0], p[1], p[2]);
    println!("  log(o, p): [{:.6}, {:.6}, {:.6}]", v[0], v[1], v[2]);
    println!("  exp(o, v): [{:.6}, {:.6}, {:.6}]", p_back[0], p_back[1], p_back[2]);
    ...
    let conv_err: f64 = p.iter().zip(back_to_lorentz.iter()).map(|(a, b)| (a - b).powi(2)).sum::<f64>().sqrt();
    println!("  Lorentz:  [{:.6}, {:.6}, {:.6}]", p[0], p[1], p[2]);
    println!("  Poincare: [{:.6}, {:.6}]", poincare_pt[0], poincare_pt[1]);
    println!("  Back:     [{:.6}, {:.6}, {:.6}]", back_to_lorentz[0], back_to_lorentz[1], back_to_lorentz[2]);
